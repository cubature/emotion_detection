...
flags.DEFINE_integer('embedding_dim', 100, 'embedding dim')
...
flags.DEFINE_integer('class_num', 5, 'class num')
...
def evaluate(model, session, data, global_steps=None, summary_writer=None):
    ...
    for step, (x, y, mask_x) in enumerate(data_helper.batch_iter(data, batch_size=FLAGS.batch_size)):
        ...
        fetches = [model.correct_num, model.final_state_fw, model.final_state_bw]
        feed_dict = {}
...
def run_epoch(model, session, data, global_steps, valid_model, valid_data, train_summary_writer, valid_summary_writer=None):
    ...
    for step, (x, y, mask_x) in enumerate(data_helper.batch_iter(data, batch_size=FLAGS.batch_size)):
        ...
        #     model.assign_new_batch_size(session, len(x))
        #     state = session.run(model.initial_state)
        # fetches = [model.cost, model.accuracy, model.train_op, model.summary, model.final_state]
        fetches = [model.cost, model.accuracy, model.train_op, model.summary, model.final_state_fw, model.final_state_bw]
        ...
        # cost, accuracy, _, summary, state = session.run(fetches, feed_dict)
        cost, accuracy, _, summary, state_fw, state_bw = session.run(fetches, feed_dict)
...
def train_step():
    ...
    train_data, valid_data, test_data = data_helper.load_data(FLAGS.max_len)
